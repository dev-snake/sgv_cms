  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tight uppercase leading-none">Thư viện Media</h1>
          <p className="text-slate-500 font-medium italic mt-2 text-sm">Quản lý toàn bộ tài sản hình ảnh và kỹ thuật của hệ thống.</p>
        </div>
        <Button 
        ...
        <div className="p-8 border-b border-slate-50 flex flex-col sm:flex-row gap-6 items-center justify-between bg-white">
          ...
          <div className="flex items-center gap-6 text-slate-400">
            <div className="flex items-center gap-2">
              <HardDrive size={16} />
              <span className="text-[10px] font-black uppercase">{images.length} FILE</span>
            </div>
          </div>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-6">
              {filteredImages.map((image, index) => (
                <div key={image.filename} className="group relative aspect-square bg-slate-50 border border-slate-100 overflow-hidden transition-all hover:shadow-xl hover:-translate-y-1">
                  <Image
                    src={image.url}
                    alt={image.filename}
                    fill
                  ...
                  <div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col">
                    ...
                    <div className="bg-slate-900/80 backdrop-blur-md p-3 space-y-1 translate-y-full group-hover:translate-y-0 transition-transform duration-300">
                      <p className="text-[8px] font-black text-white/90 truncate uppercase tracking-tighter">{image.filename}</p>
                      <p className="text-[8px] font-bold text-white/40 uppercase tracking-widest leading-none">{formatFileSize(image.size)}</p>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="flex flex-col items-center justify-center h-[400px] text-slate-300">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.2em]">Không tìm thấy ảnh nào.</p>
            </div>
          )}
        </div>
      </div>
            ...
            >
              {isUploading ? (
                <div className="flex flex-col items-center gap-4">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Đang xử lý dữ liệu...</span>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-6">
                  ...
                  <div className="space-y-2 text-center">
                    <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest">
                      Kéo thả hoặc <span className="text-brand-primary italic">Nhấn để chọn file</span>
                    </p>
