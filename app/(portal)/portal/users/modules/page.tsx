...
import { Plus, Settings, Trash2, Search, Settings2, MoreVertical, LayoutDashboard } from "lucide-react";
    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="bg-white border border-slate-100 shadow-sm rounded-sm overflow-hidden">
        <div className="p-6 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-50/30">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="icon" className="h-12 w-12 rounded-none border-slate-200" onClick={fetchModules}>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400">Thông tin Module</th>
                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-slate-400">Ngày tạo</th>
              ...
              {isLoading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    <td className="px-8 py-6"><div className="h-4 bg-slate-100 w-48 rounded"></div></td>
                    <td className="px-8 py-6"><div className="h-4 bg-slate-100 w-32 rounded"></div></td>
                    <td className="px-8 py-6 flex justify-end"><div className="h-4 bg-slate-100 w-24 rounded"></div></td>
                  </tr>
                ))
                ...
                <tr>
                  <td colSpan={3} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center gap-2 opacity-20">
                      ...
                      <p className="text-sm font-bold uppercase tracking-widest">Không có module nào</p>
                    </div>
                  </td>
                </tr>
              ) : (
                filteredModules.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="group hover:bg-slate-50/50 transition-colors">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              {isProtected && (
                                <span className="bg-slate-100 text-slate-500 text-[8px] font-black px-1.5 py-0.5 uppercase tracking-tighter">Hệ thống</span>
                              )}
                            </div>
                      ...
                      <td className="px-8 py-6">
                        <p className="text-xs font-medium text-slate-500">{new Date(item.created_at).toLocaleDateString('vi-VN')}</p>
                      </td>
                      <td className="px-8 py-6 text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48 rounded-none border-slate-100 p-1">
                            <DropdownMenuItem asChild className="rounded-none cursor-pointer focus:bg-slate-50">
                              <Link href={PORTAL_ROUTES.users.modules.edit(item.id)} className="flex items-center w-full px-3 py-2 text-xs font-bold uppercase tracking-widest text-slate-600">
                                <Settings size={14} className="mr-3" /> Chỉnh sửa
                              </Link>
                            </DropdownMenuItem>
                            <DropdownMenuItem 
                              className={cn(
                                "rounded-none cursor-pointer font-bold px-3 py-2 text-xs uppercase tracking-widest",
                                isProtected ? "opacity-30 cursor-not-allowed text-slate-400" : "focus:bg-rose-50 text-rose-500"
                              )}
                              onSelect={() => !isProtected && setDeleteId(item.id)}
                              disabled={isProtected}
                            >
                              <Trash2 size={14} className="mr-3" /> Xóa Module
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </td>
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent className="rounded-none border-slate-100 max-w-md">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-xl font-black uppercase tracking-tight text-slate-900">Xác nhận xóa Module?</AlertDialogTitle>
            <AlertDialogDescription className="text-slate-500 font-medium italic text-sm">
              Hành động này không thể hoàn tác. Các quyền hạn gắn với module này sẽ bị ảnh hưởng.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="mt-8 gap-3">
            <AlertDialogCancel className="h-12 px-8 rounded-none border-slate-200 text-[10px] font-black uppercase tracking-widest hover:bg-slate-50 transition-all">Hủy bỏ</AlertDialogCancel>
            <AlertDialogAction 
              onClick={(e) => {
                e.preventDefault();
                handleDelete();
