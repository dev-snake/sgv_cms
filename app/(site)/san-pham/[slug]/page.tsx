  ...
  if (!product) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center pt-40">
        ...
        <Link href={SITE_ROUTES.PRODUCTS} className="text-brand-primary hover:underline">Quay lại danh sách</Link>
      </div>
    );
  }
          ...
          <table className="w-full text-left border-collapse bg-white shadow-sm ring-1 ring-slate-200">
            <thead>
              <tr className="bg-slate-50">
                {headers.map((h, i) => (
                  <th key={i} className="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-muted-foreground border-b border-slate-100">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {product.tech_specs.map((row: any, i: number) => (
                <tr key={i} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
                  {headers.map((h, j) => (
                    <td key={j} className="px-8 py-6 text-sm font-bold text-slate-900">
                      {row[h]}
                    </td>
                  ))}
                </tr>
          ...
          <table className="w-full text-left border-collapse bg-white shadow-sm ring-1 ring-slate-200">
            <tbody>
              {entries.map(([label, value], i) => (
                <tr key={i} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors">
                  <td className="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-muted-foreground w-1/3 bg-slate-50/50">{label}</td>
                  <td className="px-8 py-6 text-sm font-bold text-slate-900">{String(value)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="bg-slate-50 border-b border-slate-100 py-4">
        <div className="container mx-auto px-4 lg:px-8">
           <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-muted-foreground">
              <Link href={SITE_ROUTES.HOME} className="hover:text-brand-primary">Trang chủ</Link>
              <ChevronRight size={14} />
              <Link href={SITE_ROUTES.PRODUCTS} className="hover:text-brand-primary">Sản phẩm</Link>
              <ChevronRight size={14} />
              <span className="text-brand-primary uppercase tracking-tighter">{product.name}</span>
           </div>
        </div>
      </div>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
              ...
              <div className="space-y-12">
                 <div className="space-y-6">
                    <div className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-primary">{product.category_name || product.category}</div>
                    <h1 className="text-4xl sm:text-5xl font-bold text-slate-900 tracking-tighter uppercase leading-none">{product.name}</h1>
                    <p className="text-xl text-muted-foreground font-medium leading-relaxed italic border-l-4 border-slate-100 pl-8">{product.description}</p>
                 </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8 py-10 border-y border-slate-100">
                    <div className="flex items-start gap-4">
                       ...
                       <div>
                          ...
                          <div className="text-sm font-bold text-slate-900 uppercase">{product.availability || "Liên hệ"}</div>
                       </div>
                    </div>
                    <div className="flex items-start gap-4">
                       ...
                       <div>
                          ...
                          <div className="text-sm font-bold text-slate-900 uppercase">{product.delivery_info || "Toàn quốc"}</div>
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="flex flex-col sm:flex-row gap-6 pt-6">
                    <Link href={SITE_ROUTES.CONTACT} className="flex-1 inline-flex items-center justify-center gap-4 bg-brand-primary py-5 text-[10px] font-black uppercase tracking-widest text-white shadow-xl hover:bg-brand-secondary transition-all">
                       NHẬN BÁO GIÁ CHI TIẾT
                    </Link>
                    {product.catalog_url && (
                      <a href={product.catalog_url} target="_blank" className="flex-1 inline-flex items-center justify-center gap-4 bg-white border-2 border-slate-100 py-5 text-[10px] font-black uppercase tracking-widest text-slate-900 hover:border-brand-primary transition-all">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-20">
               ...
               <div className="space-y-12">
                  <div className="bg-slate-900 p-12 text-white space-y-8 relative overflow-hidden">
                     ...
                     <p className="text-xs font-medium text-slate-400 leading-relaxed">{product.tech_summary || "Sài Gòn Valve cung cấp đầy đủ chứng chỉ CO/CQ và hỗ trợ kỹ thuật tận nơi cho các dự án trọng điểm."}</p>
                     <div className="space-y-6 pt-6 pt-b">
                        <div className="flex items-center gap-4">
                           <ShieldCheck className="text-brand-primary" size={24} />
                           <span className="text-[10px] font-bold uppercase tracking-widest">Bảo hành {product.warranty || "12 tháng"}</span>
                        </div>
                        <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-4">
                           ...
                           <span className="text-[10px] font-bold uppercase tracking-widest">Xuất xứ: {product.origin || "Chính hãng"}</span>
                        </div>
                     </div>
                  </div>
               </div>
      ...
      <section className="py-24 bg-white">
        <div className="container mx-auto px-4 lg:px-8 text-center space-y-10">
           ...
           <div className="flex justify-center flex-col sm:flex-row gap-6">
              <Link href={SITE_ROUTES.CONTACT} className="px-12 py-5 bg-brand-primary text-white text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-brand-secondary transition-all">GỬI YÊU CẦU NGAY</Link>
