          ...
          <CarouselContent>
            {products.map((product) => (
              <CarouselItem key={product.id} className="md:basis-1/2 lg:basis-1/3">
                <a href={`/san-pham/${product.slug}`} className="p-4 flex flex-col items-center text-center space-y-6 group">
                  <div className="relative aspect-square w-full max-w-[300px] transition-transform duration-500 group-hover:scale-110">
                    <Image
                      src={product.image_url || "https://via.placeholder.com/300?text=SGV"}
                      alt={product.name}
                      fill
