      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col p-0 rounded-none border-none">
        ...
        <Tabs defaultValue="library">
          <TabsList>
            <TabsTrigger 
              value="library" 
              className="h-full text-[10px] font-black uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-brand-primary rounded-none border-none shadow-none"
            >
          ...
          <TabsContent value="library" className="flex-1 overflow-auto p-6 m-0">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">Đang tải dữ liệu...</span>
              </div>
            ) : images.length === 0 ? (
                ...
                {images.map((image) => (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-2">
                       <p className="text-[8px] font-black text-white truncate uppercase">{image.filename}</p>
                       <p className="text-[8px] font-bold text-white/60 uppercase">{formatFileSize(image.size)}</p>
                    </div>
                  </button>
                ))}
              </div>
